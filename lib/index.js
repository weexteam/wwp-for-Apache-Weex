...
function processNpm (options) {
  var modules = options.npm || []

  if (modules.length <= 0) {
    return
  }
  // process npm imports
  // not to install npm modules by default.
  content += eol + '// import npm modules.' + eol
  log('npm modules-->', JSON.stringify(modules, null, 2))

  tips.npm = []
  ; (modules || []).forEach(function (mod) {
    var importName = mod.split(/[-.]/).map(function (part) {
        return part[0].toUpperCase() + part.substr(1)
      }).join('')
    exportLine += importName + ', '
    content += 'import ' + importName + ' from \'' + mod + '\'' + eol
    tips.npm.push({
      name: importName,
      path: ''
    })
  ...
  return genLocalBundleJs(jsPath)
    ...
    .then(function (bundleCode) {
      bundleCode = bundleCode
        ...
        .replace(/\r\n|\n/g, '')
      return fs.outputFileAsync(output, html.replace(bundleMark, bundleCode))
    })
}
...
function processBundle (options) {
  ...
  return fs.readFileAsync(entry, 'utf-8')
    .then(function (html) {
